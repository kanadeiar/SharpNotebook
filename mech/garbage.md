...
Поколение 1 постоянно растет. Как только превысит пороговое значение, среди него начинается уборка мусора, и все объекты из поколения 1 переходят в поколение 2. 

Управляемая куча поддерживает только три поколения. При инициализации устанавливаются пороговые значения для всех трех поколений. В процессе работы уборщик мусора CLR самонастраивается.

## Запуск уборщика

Запуск уборки мусора происходит, когда объем поколения 0 достигает порогового значения, либо вызов статического метода GC.Collect(), Windows сообщает о нехватке памяти, происходит выгрузка домена приложения, происходит завершение работы CLR.
