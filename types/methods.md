...
Конструкторы ссылочного типа - специальные методы, корректно инициализирующие новый экземпляр типа. При создании экземпляра объекта ссылочного типа выделяется память для полей данных, инициализируются служебные поля и затем вызывается конструктор, устанавливающий исходное состояние объекта.
